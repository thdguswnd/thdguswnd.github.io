...
- 배틀: 상대(우상)/아군(좌하) 배치, 기술=신혼집/신혼여행/미정/미정 OK
- 전투: HP 100→75→50→25→0 (기술당 25%), 사용 기술 비활성화, 4기술 후 승리 + CTA OK
  (※ 이후 버전에서 포켓몬 방식은 폐기, 트레이너 직접 대결로 변경됨)
...
- 배틀 인트로 + 캐릭터 순차 등장, 조사 자동(은/는·을/를) OK
- 전투: HP 감소, 사용 기술 비활성, 승리+CTA OK
- 이미지: assets/*.png 없으면 SVG 폴백(404→폴백) 정상. JS 에러 없음.
- 회귀: fade/전환을 setTimeout 기반으로(비표시 탭에서도 안정), 가로 3:2 + 세로 90° 회전 유지

## Verification Log (v3, 2026-08-18 — 전투 UI/보기 조정)
- 전투 하단을 스샷1처럼 2패널로: 좌 프롬프트 + 우 2x2 보기
- 보기 = 신혼집 어디야? / 신혼여행 어디가? / 밴드왕이 뭐야? / 도망친다
- 전투: HP 감소 + 질문 대사 조사 자동 처리 확인
